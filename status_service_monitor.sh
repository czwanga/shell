...
<caption><h2>xxx系统监控</h2></caption>
<tr><th>No.</th><th>application Name</th><th>Server IP</th><th>Port</th></tr>
EOF

WORK_DIR=/data/network_status
EMAIL="czwang@gmail.com  test@gmail.com"
cd $WORK_DIR
  ...
  #    echo $?
   if [ $? -eq 0 ] ;then
	     if [ `awk /$ADDRESSS.*down\ / $WORK_DIR/Status.txt |wc -l` -eq 1 ];then         #如果status.txt有，宕机故障恢复，则清除宕机故障记录，发送恢复通知
                echo "$(date) $SVRNAME服务,$ADDRESSS:恢复正常" | mail -s "恢复通知-宕机-$SVRNAME服务-$ADDRESSS" ${EMAIL}
         ...
         status=`nmap -P0 -n -p$PORT $ADDRESSS |grep tcp |awk '{print $2}'`
               if [[ $status = "filtered" ]] || [[ $status = "closed" ]] ;then
	            if [ `awk /$ADDRESSS.*$PORT\ / $WORK_DIR/Status.txt |wc -l` -eq 0 ];then  #如果status.txt无则记录故障（新端口故障）发送故障邮件 输出
                           echo "$(date) $SVRNAME服务,$ADDRESSS:$PORT:端口故障，请管理员及时处理！" | mail -s "故障通知-端口-$SVRNAME服务-$ADDRESSS:$PORT" ${EMAIL}
                  ...
                  if [ $status = "open" ];then
                       if [ `awk /$ADDRESSS.*$PORT\ / $WORK_DIR/Status.txt |wc -l` -eq 1 ];then  #如果status.txt有，端口故障恢复，则清除端口故障记录，发送恢复邮件，输出
                          echo "$(date) $SVRNAME服务,$ADDRESSS:$PORT:恢复正常" | mail -s "恢复通知-端口-$SVRNAME服务-$ADDRESSS:$PORT" ${EMAIL}
	   ...
	   echo "$(date) $SVRNAME服务,$ADDRESSS:宕机故障，请管理员及时处理!" | mail -s "故障通知-宕机-$SVRNAME服务-$ADDRESSS" ${EMAIL}
...
LINES=`awk 'END{print NR}' config_list.txt`                                        #获取总config_list.txt文件的条目数

i=1
while (($i<=$LINES)) 

        do
        ADDRESSS=`sed -n "$i"p config_list.txt | awk -F" " '{print $1}'`    #获取地址
        PORT=`sed -n "$i"p config_list.txt | awk -F" " '{print $2}'`        #获取端口
        SVRNAME=`sed -n "$i"p config_list.txt | awk -F" " '{print $3}'`     #获取服务名 
        check_svr
        i=$(($i+1))
done
echo -e "</table>\n</body>\n</html>\n">>$WEB_PAGE
